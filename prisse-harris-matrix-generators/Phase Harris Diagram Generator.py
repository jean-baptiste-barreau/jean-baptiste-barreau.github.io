"""
Harris Phase Diagram Generator with Second-Order Links and Overhang Clusters v5

Generates archaeological Harris diagrams with phase coloring, cluster grouping,
and second-order stratigraphic relations from CSV input files.
"""
...
# -----------------------------------------
# PHASE CONFIGURATION AND COLOR MAPPING
# -----------------------------------------
...
print("="*60)
print("HARRIS PHASE DIAGRAM GENERATOR - VERSION STABLE")
print("="*60)

# -----------------------------------------
# 1. Load phase assignments for UCs
# -----------------------------------------
uc_phases = {}
print("\n📁 Reading UC phase assignments...")
try:
    ...
    print(f"  ✓ Found {len(uc_phases)} UCs with phase assignments")
except FileNotFoundError:
    print("  ⚠️  File './noeuds.csv' not found")
    uc_phases = {}

# -----------------------------------------
# 2. Number UCs for display
# -----------------------------------------
print("\n🔢 Assigning numbers to UCs...")
uc_numbers = {}
for idx, uc in enumerate(sorted(uc_phases.keys()), start=1):
    uc_numbers[uc] = str(idx)
print(f"  ✓ Numbered {len(uc_numbers)} UCs")

# -----------------------------------------
# 3. Load cluster relations
# -----------------------------------------
edges = []
synch_edges = []
all_clusters_set = set()

print("\n📁 Reading cluster relations...")
try:
...
# -----------------------------------------
# 4. Load cluster descriptions and UC memberships
# -----------------------------------------
cluster_data = {}
print("\n📁 Reading cluster descriptions and UC memberships...")
try:
    ...
    print(f"  ✓ Found data for {len(cluster_data)} clusters")
except FileNotFoundError:
    print("  ⚠️  File './noeuds_clusters.csv' not found")

# Initialize empty entries for clusters without data
...
print(f"  ✓ UCs inside clusters: {len(ucs_in_clusters)}")
print(f"  ✓ UCs outside clusters: {len(ucs_outside_clusters)}")

# -----------------------------------------
# 6. Load UC-to-UC relations
# -----------------------------------------
uc_relations = []
print("\n📁 Reading UC-to-UC relations...")
try:
    ...
    print(f"  ✓ Found {len(uc_relations)} UC-to-UC relations")
except FileNotFoundError:
...
print(f"  ✓ {G_clusters.number_of_edges()} relations")
...
def format_uc_label(uc, phase, uc_number=None):
    """Format UC label without complex HTML"""
    bg_color = color_map.get(phase, "#FFFFFF")
    uc_clean = clean_text(uc)
    
    if uc_number:
        return f'[label="({uc_number})\\n{uc_clean}", style=filled, fillcolor="{bg_color}", shape=box, fontsize=9, fontname="Arial", penwidth=1]'
    else:
        return f'[label="{uc_clean}", style=filled, fillcolor="{bg_color}", shape=box, fontsize=9, fontname="Arial", penwidth=1]'

def format_cluster_title(cluster, data):
    """Format cluster title for display"""
    cluster_clean = clean_text(cluster)
    desc_clean = clean_text(data['description'])
    
    if desc_clean:
        return f'"Second-order link {cluster_clean}\\n{desc_clean}"'
    else:
        return f'"Second-order link {cluster_clean}"'
...
with open(dot_file, "w", encoding="utf-8") as f:
    ...
    # Dictionaries for node tracking
    uc_to_nodeid = {}
    ucs_by_cluster = {}
    
    # 1. UCs inside clusters
    ...
    # 3. Subgraphs for clusters
    f.write("  // CLUSTERS (SECOND-ORDER LINKS)\n")
    for cluster in sorted(G_clusters_red.nodes()):
    ...
    # 4. Cluster-to-cluster relations
    f.write("  // CLUSTER-TO-CLUSTER RELATIONS\n")
    ...
    # 5. Synchronous relations
    if synch_edges:
        f.write("  // SYNCHRONOUS RELATIONS\n")
        for a, b in synch_edges:
    ...
    # 6. UC-to-UC relations
    f.write("  // UC-TO-UC RELATIONS\n")
    relation_count = 0
    ...
    # Map UC to parent cluster
    uc_to_parent_cluster = {}
    for cluster, data in cluster_data.items():
        for uc in data['ucs']:
            uc_to_parent_cluster[uc] = cluster
    
    # Build rank constraints based on direct relations
    rank_constraints = set()
    
    for older, younger in uc_relations:
        # Case: older inside cluster, younger outside cluster
        ...
        # Case: older outside cluster, younger inside cluster
...
print(f"  ✓ Added {relation_count} UC-to-UC relations")
print(f"  ✓ Added {len(rank_constraints)} rank constraints")
...
print("📊 SUMMARY")
print("="*60)
print(f"- Clusters (second-order links): {len(all_clusters_set)}")
print(f"- Older/younger relations: {len(edges)}")
print(f"- Synchronous relations: {len(synch_edges)}")
print(f"- Total UCs: {len(uc_phases)}")
print(f"- UCs inside clusters: {len(ucs_in_clusters)}")
print(f"- UCs outside clusters: {len(ucs_outside_clusters)}")
print(f"- UC-to-UC relations: {len(uc_relations)}")
