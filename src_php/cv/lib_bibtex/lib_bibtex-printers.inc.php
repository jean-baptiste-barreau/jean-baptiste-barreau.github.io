...
abstract class BasePrinter {
	...
	function toggleButton($entry) {
		$l_result = (isset($entry['abstract'])) ? '<td style="vertical-align: top;"><button id="button-' . $entry['key'] . '" class="toggle-button" onclick="toggleText(\'' . $entry['key'] . '\')">+</button></td>' : '<td></td>';
		return $l_result;
	}
	
	function abstractText($entry) {
		$l_keywords = (isset($entry['keywords'])) ? $entry['keywords'] : '';
		$l_result = (isset($entry['abstract'])) ? '<div id="abstract-' . $entry['key'] . '" class="toggle-text" style="display:none;"><hr><center><b><u><span class="fr">Résumé: </span><span class="en">Abstract: </span></b></u></center>' . $entry['abstract'] . '<br><center><b><u><span class="fr">Mots-clés: </span><span class="en">Keywords: </span></b></u>' . $l_keywords. '</center></div>' : '';
	...
	function CitationStr($entry) {
		...
		$str .= str_replace('Barreau, J.B.','<b>Barreau, J.B.</b>','<span class="bibtex-author">' . ($fullnames[strlen($fullnames)-1] == '.' ? substr($fullnames, 0, -1) : $fullnames) . '.</span> ');
		...
		$str .= '<span class="bibtex-title"><a href="'.(!isset($entry['url']) ? 'http://www.google.com/search?q=' . $this->RemoveTag($entry['title']) : $entry['url'] ). '" target="_blank" class="content-link">' . $entry['title'] . '</a></span>. ';
